...
async function setup() {
	let b = document.getElementById("bars");//Reading the value from the id of this input tag 
	// console.log(b.value);
	let d = document.getElementById("delay");//Reading the value from the id of this input tag from delay 
	// console.log(d.value); 
...
async function BubbleSort() {
	...
	for (let i = 0; i < bars.length - 1; i++) {
		...
		for (let j = 0; j < bars.length - i - 1; j++) {
			...
			// console.log(bars[0].split('id="')[1].split('"')[0]);
			// console.log(bars[j].split('id="')[1].split('"')[0]);
			// console.log(curr_id);
			let nxt_ele = bars[j + 1].split('id="')[1].split('"')[0];
			// console.log(nxt_ele); 

			document.getElementById(curr_id).style.backgroundColor = selected;//selected element get yellow color
			let sound = MapRange(document.getElementById(curr_id).style.height.split('%')[0], 2, 100, 500, 1000);
			// console.log(sound);
			beep(100, sound, delay)
			document.getElementById(nxt_ele).style.backgroundColor = chng;
			...
			console.log("INside the bubble sort");
			// console.log(bars[j]);
